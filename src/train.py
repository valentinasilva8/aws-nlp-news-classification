...
csv_path = os.path.join(data_dir, "newsCorpora.csv")
...
if len(df) == 0:
    raise RuntimeError(f"No rows read from {csv_path}. Is the filename and separator correct?")

# Keep only title and category
df = df[["TITLE", "CATEGORY"]].dropna().reset_index(drop=True)
...
def train(epoch, model, device, training_loader, optimizer, loss_function):
    ...
    for _,data in enumerate(training_loader,0):
        ...
        assert outputs.dim() == 2 and outputs.size(1) == 4, f"unexpected outputs shape {outputs.shape}"
        assert targets.dim() == 1, f"unexpected targets shape {targets.shape}"
